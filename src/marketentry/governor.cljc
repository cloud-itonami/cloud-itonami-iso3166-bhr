(ns marketentry.governor
  ...
  notion of jurisdictional procurement law, whether a Bahrain-resident
  ...
  actuation is always a human call.
    ...
    2. Evidence incomplete         -- for `:filing/draft`/
                                       ...
                                       checklist on file?
    3. Bahrain-resident-rep missing  -- for `:filing/submit`, when the
                                       engagement declares
                                       `:requires-bh-entity?
                                       true` (near-universal for Bahrain
                                       ...
                                       Grounded in Bahraini commercial
                                       registration typically required for
                                       government awards (contracting
                                       authorities / Tender Board).
    ...
    (let [e (store/engagement st subject)
      ...
      (when-not (and assessment
        ...
        [{:rule :evidence-incomplete
          :detail "法域の必要書類(CR/統一資格/Tender Board e-Tendering登録/代理人確認等)が充足していない状態での提案"}]))))

(defn- bh-entity-missing-violations
  "For `:filing/submit`, when the engagement declares
  `:requires-bh-entity? true`, INDEPENDENTLY verify
  `:has-bh-entity?` is true -- the flagship genuinely new
  check this vertical adds. CONDITIONAL on the engagement's own
  `:requires-bh-entity?` ground truth (most Bahrain public
  tenders require it; a pure-foreign-market engagement may not)."
  [{:keys [op subject]} st]
  (when (= op :filing/submit)
    (let [e (store/engagement st subject)]
      (when (and (true? (:requires-bh-entity? e))
                 (not (true? (:has-bh-entity? e))))
        [{:rule :bh-entity-missing
          :detail (str subject " はバーレーン居住代理人を要するが未確認 -- 提出提案は進められない")}]))))
  ...
  (when (= op :filing/submit)
    (let [e (store/engagement st subject)]
      (when (and (true? (:requires-cr? e))
        ...
        [{:rule :cr-unverified
          :detail (str subject " はCR確認を要するが未確認 -- 提出提案は進められない")}]))))
...
(defn check
  ...
  (let [hard (into []
                   (concat (spec-basis-violations request proposal)
                           ...
                           (bh-entity-missing-violations request st)
