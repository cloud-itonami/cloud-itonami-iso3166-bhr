(ns marketentry.store
  "SSoT for the BHR market-entry compliance actor, behind a `Store`
...
(defn demo-data
  ...
  []
  {:engagements
   {"eng-1" {:id "eng-1" :operator "Kita Systems BH" :portal "Tender Board e-Tendering"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             :requires-bh-entity? true :has-bh-entity? true
             :requires-cr? true :cr-verified? true
             :drafted? false :submitted? false
             :jurisdiction "BHR" :status :intake}
    "eng-2" {:id "eng-2" :operator "Atlantis LLC" :portal "Tender Board e-Tendering"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             :requires-bh-entity? true :has-bh-entity? true
             :requires-cr? true :cr-verified? true
    ...
    "eng-3" {:id "eng-3" :operator "Minami Systems" :portal "Tender Board e-Tendering"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             ...
             :drafted? false :submitted? false
             :jurisdiction "BHR" :status :intake}
    "eng-4" {:id "eng-4" :operator "Higashi Export" :portal "Tender Board e-Tendering"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             :requires-bh-entity? true :has-bh-entity? false
             :requires-cr? true :cr-verified? true
             :drafted? false :submitted? false
             :jurisdiction "BHR" :status :intake}
    "eng-5" {:id "eng-5" :operator "Nishi Logistics" :portal "Tender Board e-Tendering"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             :requires-bh-entity? true :has-bh-entity? true
             ...
             :drafted? false :submitted? false
             :jurisdiction "BHR" :status :intake}
    "eng-6" {:id "eng-6" :operator "Chuo Civic Tech" :portal "Tender Board e-Tendering"
...
(defn- engagement->tx [{:keys [id operator portal base-fee monthly-rate monitoring-months claimed-fee
                               requires-bh-entity? has-bh-entity?
  ...
  (cond-> {:engagement/id id}
    ...
    (some? requires-bh-entity?)  (assoc :engagement/requires-bh-entity? requires-bh-entity?)
    (some? has-bh-entity?)       (assoc :engagement/has-bh-entity? has-bh-entity?)
...
(def ^:private engagement-pull
  [:engagement/id :engagement/operator :engagement/portal :engagement/base-fee :engagement/monthly-rate
   ...
   :engagement/requires-bh-entity? :engagement/has-bh-entity?
...
(defn- pull->engagement [m]
  (when (:engagement/id m)
    {:id (:engagement/id m) :operator (:engagement/operator m) :portal (:engagement/portal m)
     ...
     :requires-bh-entity? (boolean (:engagement/requires-bh-entity? m))
     :has-bh-entity? (boolean (:engagement/has-bh-entity? m))
...
(defn kotobase-store
  ...
  [{:keys [db-api seq-fn]}]
  (->KotobaseStore (persist/store {:db-api db-api :actor "marketentry-bhr"})
